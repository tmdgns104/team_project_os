const state = {
  projects: [], projectId: null, snapshot: null, view: 'overview', ws: null,
  accessKey: localStorage.getItem('project_os_access_key') || ''
};
const titles = {
  overview:'Overview', definition:'Goal & Requirements', progress:'Development Progress',
...
function statusChip(status){
  const map={done:['완료','good'],in_progress:['진행중','ai'],review:['검토','warn'],todo:['예정',''],blocked:['차단','danger'],defined:['정의됨','good'],accepted:['승인','good'],open:['Open',''],discussing:['논의중','warn'],queued:['대기','warn'],claimed:['실행중','ai'],completed:['완료','good'],failed:['실패','danger']};
...
async function loadProjects(){
  ...
  if(state.projectId){ await loadSnapshot(); connectWs(); }
}
...
function render(){
  if(!state.snapshot){ $('#content').innerHTML='<div class="empty">프로젝트를 불러오는 중...</div>'; return; }
  const fn={overview:renderOverview,definition:renderDefinition,progress:renderProgress,process:()=>renderDiagram('process'),architecture:()=>renderDiagram('architecture'),dataflow:()=>renderDiagram('dataflow'),ideas:renderIdeas,team:renderTeam}[state.view];
...
function renderTeam(){
  ...
  <div><div class="ai-box"><strong>Bring Your Own AI</strong><p style="color:#aeb9cc">각 팀원이 자기 PC의 Codex / Claude Code를 Local Bridge로 연결합니다. AI가 없어도 일반 팀원 기능은 동일합니다.</p><button class="mini-btn" data-action="bridge-help">Local Bridge 연결 방법</button></div><div class="panel" style="margin-top:18px"><h3>연결된 Bridge</h3>${s.bridges.length?s.bridges.map(b=>`<div class="member-card"><strong>${esc(b.member_name)} · ${esc(b.provider)}</strong><div><small>${esc(b.machine_name)} · 최근 연결 ${new Date(b.last_seen).toLocaleString('ko-KR')}</small></div></div>`).join(''):'<div class="empty">아직 연결된 AI Bridge가 없습니다.</div>'}</div></div></div>
...
function openAddForView(){ handleAction({overview:'add-task',definition:'add-requirement',progress:'add-task',process:'add-node',architecture:'add-node',dataflow:'add-node',ideas:'add-idea',team:'add-member'}[state.view],['process','architecture','dataflow'].includes(state.view)?state.view:null); }
function handleAction(action, view){
  if(action==='add-task') return addTask(); if(action==='add-requirement') return addRequirement(); if(action==='edit-goal') return editGoal();
...
function addMember(){ openModal('팀원 추가',field('name','이름')+field('role','역할','Developer')+selectField('ai_provider','개인 AI',[['none','AI 사용 안 함'],['codex','Codex'],['claude','Claude Code'],['opencode','OpenCode'],['ollama','Ollama']],'none'),fd=>api(`/api/projects/${state.projectId}/members`,{method:'POST',body:JSON.stringify(Object.fromEntries(fd))})); }
function addAIJob(){ const tasks=state.snapshot.tasks.filter(t=>t.status!=='done'); const members=state.snapshot.members.filter(m=>m.ai_provider!=='none'); if(!tasks.length||!members.length){toast('미완료 Task와 AI 사용 팀원이 필요합니다.');return;} openModal('개인 AI에 Task 전달',selectField('task_id','Task',tasks.map(t=>[String(t.id),`#${t.id} ${t.title}`]))+selectField('member_name','팀원',members.map(m=>[m.name,`${m.name} (${m.ai_provider})`]))+selectField('provider','Provider',[['codex','Codex'],['claude','Claude Code'],['opencode','OpenCode'],['ollama','Ollama']],'codex')+field('repo_hint','로컬 Repository 경로 힌트','')+textarea('instruction','추가 지시','현재 Task 범위만 수행하고 변경 내용과 검증 결과를 남겨주세요.'),fd=>{const o=Object.fromEntries(fd);o.task_id=Number(o.task_id);return api(`/api/projects/${state.projectId}/ai-jobs`,{method:'POST',body:JSON.stringify(o)})}); }
function bridgeHelp(){
  const cmd=`python local_bridge/bridge.py register --server ${location.origin} --project ${state.projectId} --member "내 이름" --provider codex`;
  openModal('Local Bridge 연결',`<div class="notice">1) 프로그램을 팀원 PC에 내려받고 2) 개인 AI CLI에 로그인한 뒤 3) 아래 명령으로 Bridge를 등록합니다.</div><div class="code-line">${esc(cmd)}</div><div class="notice">등록 시 발급된 token을 사용해 <b>run</b> 명령을 실행하면 해당 팀원에게 배정된 AI Task를 가져옵니다. 자세한 내용은 README의 BYOAI 항목을 참고하세요.</div>`,async()=>({}));
}
init();
